...
const Resume = () => {
    ...
    const downloadPdf = () => {
        ...
        const applyStylesRecursively = (element, styles) => {
            const children = element.getElementsByTagName('*');
            for (let i = 0; i < children.length; i++) {
                const child = children[i];
                for (const key in styles) {
                    const originalStyle = window.getComputedStyle(child)[key];
                    const originalValue = parseFloat(originalStyle);
                    const unit = originalStyle.replace(originalValue, '');
                    child.style[key] = `${originalValue + 2}${unit}`; // Double the original value
        ...
        for (let i = 0; i < elements.length; i++) {
            originalStyles.push({
                element: elements[i],
                fontSize: elements[i].style.fontSize,
            });
        }

        // Apply the styles for the PDF
        applyStylesRecursively(input, { fontSize: 'fontSize' });
        ...
        html2canvas(input, {
            ...
            // Revert the styles back to the original
            for (let i = 0; i < originalStyles.length; i++) {
                originalStyles[i].element.style.fontSize = originalStyles[i].fontSize;
            }
        }).catch(error => {
    ...
    return !loading ? (
        <div>
            <article className="resume-wrapper text-center position-relative printablediv" ref={contentRef}>
            ...
            <div className="floatbtn">
                <button className={classname} onClick={() => { setClassname("social hide"); window.print() }}> <i className="bi bi-printer-fill"></i></button>
                <button className={classname} onClick={downloadPdf}><i className="bi bi-download"></i></button>
                <button className={close} onClick={Toggle} href="#"><i className="bi bi-plus"></i></button>
            </div>
        </div>
